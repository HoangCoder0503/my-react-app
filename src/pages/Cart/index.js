...
import { updateCart } from "../../redux-setup/reducers/cart";
import { deleteItemCart } from "../../redux-setup/reducers/cart";
...
const Cart = () => {
  ...
  const changeQty = (e, id) => {
    ...
    if (value === 0) {
      ...
      const isConfirm = confirm("Bạn muốn xóa sản phẩm này khỏi giỏ hàng");
  ...
  const newItemCart = itemsCart.map((item) => ({
    prd_id: item._id,
    price: item.price,
    qty: item.qty
  }));
      ...
      <div id="my-cart">
        ...
        <form method="post">
            ...
            itemsCart.map((item, index) => (
              <div key={index} className="cart-item row">
                ...
                <div className="cart-price col-lg-3 col-md-3 col-sm-12">
                  ...
                  <a onClick={(e) => clickDeleteItemCart(e, item._id)} href="#">Xóa</a>
                </div>
              </div>
            ))
          }
          ...
          <div className="row">
            ...
            <div className="cart-price col-lg-3 col-md-3 col-sm-12">
              <b>{formatPrice(itemsCart.reduce((total, item) => total + item.qty * item.price, 0))}</b>
            </div>
          </div>
        </form>
      </div>
