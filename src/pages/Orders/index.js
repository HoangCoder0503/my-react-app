...
import { orderCanceled } from "../../services/Api";
const Orders = () => {
                ...
                <form method="post">
                    {
                        orders.map((item, index) =>
                            <div key={index} className={`cart-item row ${item.status===0? "alert-danger" : ""} ${item.status===2? "alert-success" : ""}`}>
                                <div className="cart-thumb col-lg-7 col-md-7 col-sm-12">
                                    <h4>Đơn hàng đã mua vào ngày:
                                ...
                                <div className="cart-price col-lg-2 col-md-2 col-sm-12"><b>{formatPrice(item.totalPrice)}</b></div>
                                <div className="cart-quantity col-lg-3 col-md-3 col-sm-12">
                                    <Link to={`/OrderDetails-${item._id}`} className="btn btn-outline-dark mb-1">Chi tiết đơn hàng</Link>
                                    {
                                        item.status===2
                                    ...
                                    }
                                    {
                                        item.status === 1
                                            ? (<>
                                                <button onClick={()=> clickOrderCancel(item._id)} type="button" className="btn btn-outline-danger mb-1">Huỷ đơn</button>
