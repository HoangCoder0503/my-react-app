...
export const PaginationCategory = ({ pages }) => {
    const { id } = useParams();
    const {
        total,
        totalPages,
    ...
    const formatUrl = (sotrang) => {
        return `/Category-${id}?page=${sotrang}`;
    }
    const clickPage = (e, sotrang) => {  // trang là biến đặt tên tùy ý để nhận giá trị của biến item ở dưới vào.
        if (sotrang === "...") return e.preventDefault();
    };

    const renderProductsCategoryPages = (delta = 2) => {
        // mang [1... 3 4 (5) 6 7 ... 10].
        const pagesHtml = [];
    ...
    return (
        <>
            <ul className="pagination">
                ...
                {
                    renderProductsCategoryPages().map((item, index) => (
