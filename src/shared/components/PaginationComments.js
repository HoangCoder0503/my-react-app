...
export const PaginationComments = ({ pages }) => {
    const { id } = useParams();
    const {
        total,
        totalPages,
    ...
    const formatUrl = (sotrang) => {
        return `/ProductDetails-${id}?page=${sotrang}`;
    }
    const clickPage = (e, sotrang) => {  // sotrang là biến đặt tên tùy ý để nhận giá trị của biến item ở dưới vào.
        if (sotrang === "...") return e.preventDefault();
    };

    const renderCommentPages = (delta = 2) => {
        // mang [1... 3 4 (5) 6 7 ... 10].
        const pagesHtml = [];
    ...
    return (
        <>
            <ul className="pagination">
                ...
                {
                    renderCommentPages().map((item, index) => (
